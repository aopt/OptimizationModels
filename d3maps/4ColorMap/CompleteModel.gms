$ontext
  ...
  Files:
      CompleteModel.gms (this file)
      data.inc (data file)
      geojson-counties-fips.js  (geojson file)
  Output:
      results.js  (GAMS/Cplex solution)
      USCountyMap2.html (HTML/JS)

  Warning: the GAMS model is somewhat large/complex so
    so that will take a while (a few minutes).
...
variable
   ...
   maxCount
   minCount
   Count(c)  'number of counties colored with c'
;

equations
   objective            'make number of times each color is used as equal as possible'
...
counting(c).. Count(c) =e= sum(n, x(n,c));
