...
*	Set the state USPS identifier (used for geo data) and state FIPS code:

$set st wi
$set STFIPS 55


*   Use the projection for Central Wisconsin suggested by:

*	https://github.com/veltman/d3-stateplane

$set projection geoConicConformal().parallels([44 + 15 / 60, 45 + 30 / 60])
...
$set resultfile %st%results.js

set	%st%(FIPS)	Counties in %st%;
%st%(FIPS) = yes$(ord(FIPS.TL,1)=ord("%STFIPS%",1) and ord(FIPS.TL,2)=ord("%STFIPS%",2));
...
$set htmlfile %st%Map.html
$set title    County-Level %st% map using random data
...
<script src="%st%results.js"></script>
...
<script>
	...
	.rotate([90, 0]).fitExtent([[margin, margin], [width-margin, height-margin]], geodata);
